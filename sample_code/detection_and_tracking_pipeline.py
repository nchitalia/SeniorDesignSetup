...
def parse_args(argv):
    ...
    geometric_options = parser.add_argument_group("Geometric options")
    geometric_options.add_argument("--camera_roi", nargs='+', type=int, default=None,
                                   help="Camera ROI [x, y, width, height].")
    geometric_options.add_argument(
        "--transpose_input", action='store_true',
        help="""Apply transpose to input. This transformation is applied first. Note: this transformation changes input dimensions. Combine with --flipX (resp. --flipY) to rotate 90 degrees clockwise (resp. counter-clockwise)""")
    geometric_options.add_argument(
        "--flipX", action='store_true',
        help="""Flip events horizontally (combine with --transpose-input to rotate 90 degrees clockwise, or with --flipY to rotate 180 deg)""")
    geometric_options.add_argument(
        "--flipY", action='store_true',
        help="""Flip events vertically (combine with --transpose-input to rotate 90 degrees counter-clockwise, or with --flipX to rotate 180 deg)""")
    ...
    if args.camera_roi is not None:
        assert len(args.camera_roi) == 4
